...
def Lqplots(setq,the,lambd,seed,pp,reg,classifier,headings,epsa,step_size,**kwargs):
    ...
    for q in setq:
        ...
        if reg in ['lq1','lq12']:
            setjac,Bal_acc[s-1],idex[s-1] = classification(pp,the,classifier,reg,lambd,seed,step_size,**{'q':q,'epsa':epsa})
    ...
    # Plot Avg. Genes
    axs[1].plot(idex, '-*b', lw=5, markersize=20, label=the)
    ...
    axs[1].set_ylabel('Features', fontsize=16)
    axs[1].legend(fontsize=14)

    _,_,_,_,_,_,_,_,pc = getdata(pp)
    plt.suptitle('{:}'.format(pc), fontsize=20)
