...
def top_features(label, runs_list,**kwargs):
    ...
    combined_df = pd.concat([result_df1,result_df2],axis=1).fillna('').map(convert_to_integer)
    #combined_df = pd.concat([result_df1.iloc[0:20,:],result_df2.iloc[0:20,:]],axis=1).fillna('').map(convert_to_integer)
